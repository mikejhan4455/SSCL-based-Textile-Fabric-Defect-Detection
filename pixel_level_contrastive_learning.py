...
class ConvMLP(nn.Module):
    ...
    def forward(self, x):
        # NOTE: modified
        return self.net(x.permute(2, 0, 1).unsqueeze(0))


class PPM(nn.Module):
    def __init__(self, *, chan, num_layers=1, gamma=2):
        super().__init__()
        self.gamma = gamma
...
class NetWrapper(nn.Module):
    ...
    def _register_hook(self):
        ...
        assert (
            instance_layer is not None
        ), f"hidden layer ({self.layer_instance}) not found"
    ...
    def get_representation(self, x):
        ...
        self.hidden_instance = None
        assert (
            hidden_pixel is not None
        ), f"hidden pixel layer {self.layer_pixel} never emitted an output"
        assert (
            hidden_instance is not None
        ), f"hidden instance layer {self.layer_instance} never emitted an output"
        return hidden_pixel, hidden_instance

    # @autocast()
    def forward(self, x):
        pixel_representation, instance_representation = self.get_representation(x)
        instance_representation = instance_representation[0].flatten(1)
...
class PixelCL(nn.Module):
    def __init__(
        ...
        if use_pixpro:
            self.propagate_pixels = PPM(
                chan=projection_size, num_layers=ppm_num_layers, gamma=ppm_gamma
            )
    ...
    def forward(self, x, return_positive_pairs=False):
        ...
        image_one_cutout = cutout_and_resize(
            x, cutout_coordinates_one, mode=self.cutout_interpolate_mode
        )
        image_two_cutout = cutout_and_resize(
            x, cutout_coordinates_two, mode=self.cutout_interpolate_mode
        )
        ...
        image_one_cutout, image_two_cutout = self.augment1(
            image_one_cutout
        ), self.augment2(image_two_cutout)
        ...
        coordinates = torch.meshgrid(
            torch.arange(image_h, device=device), torch.arange(image_w, device=device)
        )
        ...
        proj_coors_one_expanded = (
            proj_coors_one[:, None]
            .expand(num_pixels, num_pixels, -1)
            .reshape(num_pixels * num_pixels, 2)
        )
        ...
        loss_instance_one = loss_fn(
            pred_instance_one, target_proj_instance_two.detach()
        )
        loss_instance_two = loss_fn(
            pred_instance_two, target_proj_instance_one.detach()
        )
        ...
        if not self.use_pixpro:
            ...
            loss_pix_one_two = -torch.log(
                similarity_one_two.masked_select(positive_mask_one_two[None, ...])
                .exp()
                .sum()
            ...
            loss_pix_two_one = -torch.log(
                similarity_two_one.masked_select(positive_mask_two_one[None, ...])
                .exp()
                .sum()
